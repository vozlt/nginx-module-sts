...
uintptr_t
ngx_http_stream_server_traffic_status_escape_json(u_char *dst, u_char *src, size_t size)
{
    u_char      ch;
    ngx_uint_t  len;
...
ngx_int_t
ngx_http_stream_server_traffic_status_escape_json_pool(ngx_pool_t *pool,
...
#if !defined(nginx_version) || nginx_version < 1007009
    p = (u_char *) ngx_http_stream_server_traffic_status_escape_json(p, dst->data, dst->len);
#else
...
ngx_int_t
ngx_http_stream_server_traffic_status_copy_str(ngx_pool_t *pool,
    ngx_str_t *buf, ngx_str_t *dst)
{
    u_char  *p;
...
ngx_int_t
ngx_http_stream_server_traffic_status_replace_chrc(ngx_str_t *buf,
    u_char in, u_char to)
{
    size_t   len;
    u_char  *p;
...
ngx_int_t
ngx_http_stream_server_traffic_status_replace_strc(ngx_str_t *buf,
